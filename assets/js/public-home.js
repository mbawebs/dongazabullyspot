...
function renderDogFeature(row, index, config) {
  const displayName = String(row.home_name ?? row.name ?? "").trim();
  const name = formatName(displayName, config.fallbackName);
  const lineage = String(row.lineage ?? "").trim();
  const description = String(row.home_description ?? row.description ?? "").trim();
  ...
  const imageHtml = imageBlock(row, index, config.galleryPrefix, displayName || config.fallbackName);
...
function renderProductionCard(row, index) {
  const displayName = String(row.home_name ?? row.name ?? "").trim();
  const name = formatName(displayName, `Production ${index + 1}`);
  const description = String(row.home_description ?? row.description ?? "").trim();
  const buttonText = index === 1 ? "See More" : "View Productions";

  return `
    <div class="card">
      ${cardImage(row, index, "home_production", displayName || `Production ${index + 1}`)}
      <div class="card-content">
        <h3>${name}</h3>
        ${description ? `<p>${escapeHtml(description)}</p>` : ""}
        <br>
        <a class="btn gold" href="productions.html">${buttonText}</a>
      </div>
    </div>`;
}


function renderHomeLatestGalleryCard() {
  return `
    <div class="card">
      <img src="assets/gallery/gallery_01.png" alt="Gallery">
      <div class="card-content">
        <h3>GALLERY</h3>
        <p>A closer look at dogs, productions and kennel life.</p>
        <br>
        <a class="btn gold" href="gallery.html">Open Gallery</a>
      </div>
    </div>`;
}
...
async function replaceExistingSection(containerId, table, renderer) {
  ...
  if (containerId === "homeLatestProductions") {
    container.insertAdjacentHTML("beforeend", renderHomeLatestGalleryCard());
  }
}
